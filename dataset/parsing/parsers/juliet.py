...
GCC_PREPROCESS_COMMAND = "gcc -E {} -O0 -DOMITGOOD -DINCLUDEMAIN -I{} \
            -o {}" + COMMAND_SUPRESS_OUTPUT
GPP_PREPROCESS_COMMAND = "g++ -E {} -O0 -DOMITGOOD -DINCLUDEMAIN -I{} \
             -o {}" + COMMAND_SUPRESS_OUTPUT
...
ADITIOAL_GCC_SOURCES = " " + MAIN_DATASET_HEADERS + "io.c " + \
    MAIN_DATASET_HEADERS + "std_thread.c "
ADITIOAL_GPP_SOURCES = " " + MAIN_DATASET_HEADERS + "io.cpp " + \
    MAIN_DATASET_HEADERS + "std_thread.cpp "
...
class CNistJulietParser(BaseParser):
    ...
    def preprocess(self) -> None:
        ...
        for source in sources:
            ...
            # ID of the source)
            full_source_id = DATASET_NAME + "_" + str(source.id)
            ...
            for source_complete_filename in source.full_filenames:
                # Preprocess
                ...
                gcc_command = ""
                if file_extension == ".h":
                    shutil.copy(source_complete_filename,
                                MAIN_DATASET_HEADERS + source_filename)
                else:
                    if file_extension == ".c":
                        gcc_command = GCC_PREPROCESS_COMMAND.format(
                            source_complete_filename, MAIN_DATASET_HEADERS,
                            destination_file)
                    else:
                        gcc_command = GPP_PREPROCESS_COMMAND.format(
                            source_complete_filename, MAIN_DATASET_HEADERS,
        ...
        for testcase in root.findall("./testcase"):
            try:
                ...
                for file in testcase.findall("./file"):
                    ...
                    filePath = glob.iglob(DATASET_SOURCES_FOLDER + "**/" +
                                          file.attrib["path"],
                                          recursive=True)
                ...
                self._current_id += 1
                source = SourceDetails(id, "", cwes[id], filePaths)
                sources.append(source)
                if (id == self._source_limit):
                    return sources
    ...
    def _generate_gcc_command(
        ...
        if binary_type == ".c":
            gcc_command = GCC_BUILD_COMMAND.format(
                compile_flags, sources + ADITIOAL_GCC_SOURCES, link_flags,
                destination_file)
        else:
            gcc_command = GPP_BUILD_COMMAND.format(
                compile_flags, sources + ADITIOAL_GPP_SOURCES, link_flags,
                destination_file)
